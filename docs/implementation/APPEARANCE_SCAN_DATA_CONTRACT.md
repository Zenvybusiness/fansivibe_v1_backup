# REAL Appearance Scan Data Contract

**Repository:** fansivibe_v1_backup
**Stage:** STEP 10 — Data design only
**Date:** 2026-08-15
**Rule:** DO NOT modify application code, database, API contracts, dependencies, AI models, or UI.

---

## STEP 1 — APPEARANCE DOMAIN DATA

Based strictly on the audit and existing domain model, the appearance information Fansivibe actually needs is derived from the existing `AppearanceProfile` value object, the `AnalysisRun` model, and the decision engine. No new attributes are invented.

### 1.1 Separated Attribute Taxonomy

| Category | Attribute | Type | Nullable | Allowed Values | Confidence | Source | Persisted | User-Editable | Inferred | Used by Decision Engine |
|---|---|---|---|---|---|---|---|---|---|---|
| **OBSERVED** | `faceShape` | str | nullable | vocab: `oval`, `round`, `square`, `heart`, `diamond`, `rectangular` | derived | image analysis (or profile) | yes (in `result`) | no (system-derived) | yes | yes |
| **OBSERVED** | `skinTone` | str | nullable | vocab: warm/cool + level codes | derived | image analysis (or profile) | yes (in `result`) | no | yes | yes |
| **OBSERVED** | `bodyType` | str | nullable | vocab: `slim`, `average`, `curvy`, `plus` | derived | image analysis (or profile) | yes (in `result`) | no | yes | yes |
| **OBSERVED** | `styleType` | str | nullable | vocab: 6 `StyleVibe` codes from onboarding | user-pref + derived | user selection + image | yes (in `result`) | yes (user-chosen) | yes | yes |
| **INFERRED** | `sourceRunId` | str (UUID) | nullable | any valid run UUID | n/a | producing run | yes (in `result` + `style_profile`) | no | yes | yes |
| **USER-PROVIDED** | `styleType` override | str | nullable | any `StyleVibe` code | n/a | user PATCH `/users/me` | yes (in `style_profile`) | yes | no | yes |
| **DERIVED** | `confidence` | float [0,1] | not nullable | computed | computed | decision engine | yes (in `result`) | no | yes | yes |
| **DERIVED** | `needs_more_data` | bool | not nullable | true/false | computed | decision engine | yes (in `result`) | no | yes | yes |
| **DERIVED** | `appearance` snapshot | dict | not nullable | structured | computed | decision engine + `HairstyleResult`/`GroomingResult` | yes (in `analysis_runs.result`) | no | yes | yes |

### 1.2 Attribute Origin Summary

- **OBSERVED** (`faceShape`, `skinTone`, `bodyType`): Extracted from captured image via image analysis pipeline (P1). These are the primary "observed" attributes that justify the appearance scan.
- **OBSERVED** (`styleType`): User-selected from onboarding `StyleVibe` options; can be updated via `PATCH /v1/users/me`. Also reinforced by image analysis.
- **INFERRED** (`sourceRunId`): The `run_id` of the analysis run that produced these attributes. Provenance link — never client-supplied.
- **USER-PROVIDED** (`styleType` override): The only user-editable attribute; flows through `PATCH /v1/users/me` with `sourceRunId` requirement (C-16, BAR-0).
- **DERIVED** (`confidence`, `needs_more_data`): Deterministic engine output — 50% data completeness + 50% top-pick decisiveness. Never fabricated.
- **DERIVED** (`appearance` snapshot): Serialized `AppearanceProfile` + engine output (`top`, `alternatives`, `confidence`, `needs_more_data`). Immutable once written (PR-6).

### 1.3 Key Design Decisions

1. **No biometric or sensitive attributes** beyond face-shape/vocab codes and skin-tone/vocab codes. These are the only appearance attributes supported by the existing domain model (`FaceProfile` value object).
2. **`confidence` and `needs_more_data`** are derived deterministically from the `AppearanceProfile` completeness — no LLM or external model required.
3. **`sourceRunId`** is the sole provenance mechanism — links the persisted profile back to the producing analysis run.
4. **`styleType`** is the only user-editable attribute; all other appearance fields are system-derived from images or profile.
5. **All appearance data lives inside `analysis_runs.result` JSONB** as the `AppearanceProfile` snapshot — no separate table. The `user_state.style_profile` also carries `face_shape`, `skin_tone`, `body_type`, `style_type` + `source_run_id`.
6. **`HairstyleResult` and `GroomingResult`** both embed an `AppearanceProfile`; the image-derived appearance populates this snapshot when the analysis run completes.

### 1.4 Observed vs Inferred vs User-Provided vs Derived Matrix

| Attribute | Observed | Inferred | User-Provided | Derived |
|---|---|---|---|---|
| `faceShape` | ✓ (from image) | | | ✓ (engine completeness) |
| `skinTone` | ✓ (from image) | | | ✓ (engine completeness) |
| `bodyType` | ✓ (from image) | | | ✓ (engine completeness) |
| `styleType` | ✓ (from profile/image) | | ✓ (PATCH /users/me) | |
| `sourceRunId` | | ✓ (producing run) | | |
| `confidence` | | | | ✓ (deterministic engine) |
| `needs_more_data` | | | | ✓ (deterministic engine) |
| `appearance` snapshot | | | | ✓ (HairstyleResult/GroomingResult) |

---

## STEP 2 — DATABASE DESIGN

The existing PostgreSQL schema already supports the required data. No new tables or columns are needed beyond what exists for `analysis_runs` and `user_state`. The existing structures safely represent all required information.

### 2.1 Existing Tables — Already Sufficient

| Table | Column | Type | Nullable | FK | Ownership | Index | Reason |
|---|---|---|---|---|---|---|---|
| `analysis_runs` | `id` | UUID | NOT NULL | PK | owner (user_id) | PK index | Primary scan/analysis record |
| `analysis_runs` | `user_id` | UUID | NOT NULL | FK → users | owner | index | Owner scoping (OW-1) |
| `analysis_runs` | `run_type` | text | NOT NULL | FK → run_types | system | CHECK constraint | `outfit`/`hairstyle`/`grooming` |
| `analysis_runs` | `status` | text | NOT NULL | CHECK | system | CHECK constraint | `pending`/`completed`/`failed` |
| `analysis_runs` | `engine_version` | text | NOT NULL | none | system | — | Provenance (PR-6) |
| `analysis_runs` | `input_media` | jsonb | NULL | none | system | — | `MediaRef` to source image (PR-8) |
| `analysis_runs` | `result` | jsonb | NULL | none | system | — | `AppearanceProfile` + engine snapshot (PR-6) |
| `analysis_runs` | `created_at` | timestamptz | NOT NULL | none | system | — | Run start |
| `analysis_runs` | `completed_at` | timestamptz | NULL | none | system | — | Run end |
| `user_state` | `style_profile` | jsonb | NOT NULL | FK → users (1:1) | owner | index | `face_shape`, `skin_tone`, `body_type`, `style_type`, `source_run_id` |
| `run_types` | `code` | text | NOT NULL | PK | system | PK index | `outfit`/`face`/`hairstyle`/`grooming` |

### 2.2 No New Tables Required

The audit confirms that all required data can be stored in existing structures:

1. **`analysis_runs.result` (jsonb)** — Already stores `AppearanceProfile.to_snapshot()` which includes `faceShape`, `skinTone`, `bodyType`, `styleType`, `sourceRunId`, plus `confidence`, `needs_more_data`, and recommendation data. No column addition needed.

2. **`analysis_runs.input_media` (jsonb)** — Already stores `MediaRef` (path/hash/mime/type). No column addition needed.

3. **`user_state.style_profile` (jsonb)** — Already stores `face_shape`, `skin_tone`, `body_type`, `style_type` as sparse keys plus `source_run_id`. No column addition needed.

4. **No duplicate tables** — The domain model explicitly states: "No SQL, no tables" for appearance attributes beyond what exists. The `AppearanceProfile` value object is the canonical schema; it is serialized to JSONB and stored in `analysis_runs.result` and `user_state.style_profile`.

### 2.3 Migration Assessment

**No migration is required.** The existing schema safely represents all required information because:

- The `analysis_runs.result` JSONB column already accepts the `AppearanceProfile` snapshot structure.
- The `analysis_runs.input_media` JSONB column already accepts `MediaRef` structures.
- The `user_state.style_profile` JSONB column already stores the four appearance attributes plus provenance.
- The `run_types` vocabulary already includes `outfit`/`hairstyle`/`grooming` (and reserved `face`).
- The `status` CHECK constraint already enforces `pending`/`completed`/`failed`.

**Only if image upload is added:** The `input_media` column would receive a `MediaRef` from the upload-then-insert flow (TRX-1), but the column structure already exists.

### 2.4 Index Recommendation

No new indexes required. Existing indexes on `analysis_runs(user_id, run_type, status)` and `user_state(user_id)` are sufficient for owner-scoped queries.

---

## STEP 3 — ANALYSIS RUN CONTRACT

The existing `analysis_runs` lifecycle supports the Appearance Scan. No second scan/run lifecycle is needed.

### 3.1 Run Type

- **`run_type`** ∈ {`outfit`, `hairstyle`, `grooming`} — the `face` code is reserved, no endpoint.
- For Appearance Scan: `run_type = "outfit"` (image-based scan producing appearance attributes) or `run_type = "hairstyle"` (face image → appearance + recommendations in one run).
- The `run_types` vocabulary already supports these codes (TABLE_DEFINITIONS.md §5.7).

### 3.2 Initial Status

- **`status = "pending"`** — run row created, job queued. Both CREATED and PROCESSING surface as `pending` on the wire (no separate in-flight state).

### 3.3 Processing States

- **`pending`** → job queued, not yet started, or in flight (status derived from row, no stored flag).
- **`completed`** → `result` + `engine_version` written once (TRX-5 guard).
- **`failed`** → typed `error` written, no `result`. Immutable historical record.

### 3.4 Completion State

- **`status = "completed"`** — `result` and `engine_version` written atomically. The `result` is an immutable snapshot (PR-6: never overwritten in place).
- The completed `result` contains: `AppearanceProfile` snapshot + `confidence` + `needs_more_data` + recommendations (top/alternatives).

### 3.5 Failure State

- **`status = "failed"`** — `error` body written, no `result`. The failure is itself part of history (reproducible). Not retried in place; a new submission creates a new run.

### 3.6 Retry Behavior

- **Server-side**: 1 automatic retry on transient failures (timeout, 5xx, network) in the job runner. A job that fails twice is marked `failed` permanently.
- **Client-initiated**: Re-submission creates a **new run** — never a mutation of the failed one. The failed run stays as auditable history.

### 3.7 Ownership

- Every `analysis_run` is **owner-scoped** (OW-1): `user_id` FK → `users.id`, enforced via `404-not-403` on read/detail.
- The `input_media` `MediaRef` is owner-scoped: `users/{user_id}/scans/{run_id}/input.{ext}`.

### 3.8 Input / Context Snapshot

At run creation, the following is captured:

| Field | Source | Stored As |
|---|---|---|
| `user_id` | Auth token → `deps.py` | `analysis_runs.user_id` |
| `run_type` | Endpoint path (`/outfit`/`/hairstyle`/`/grooming`) | `analysis_runs.run_type` |
| `input_media` | Inline `multipart/form-data` `image` part → `MediaRef` (TRX-1) | `analysis_runs.input_media` (jsonb) |
| `engine_version` | Fixed string (e.g. `"vision-v1"` or `"rules-v1"`) | `analysis_runs.engine_version` |
| `status` | `"pending"` | `analysis_runs.status` |
| `created_at` | `now()` | `analysis_runs.created_at` |

### 3.9 Result Reference

After completion, `analysis_runs.result` carries:
...
The `result` is the **single source of truth** for the appearance data — it feeds the decision engine and persists the profile. The `sourceRunId` within the appearance snapshot links back to the producing run.

### 3.10 Lifecycle Mapping

| Task Vocabulary | Wire Status | When |
|---|---|---|
| CREATED | `pending` | Run row exists; job queued |
| PROCESSING | `pending` | Job in flight (derived from row) |
| COMPLETED | `completed` | `result` + `engine_version` written once |
| FAILED | `failed` | Typed `error`; no `result` |

---

## STEP 4 — MEDIA CONTRACT

Design how the captured image moves through the system.

### 4.1 Flutter → Upload

| Step | Description |
|---|---|
| **Capture** | `CameraController.takePicture()` saves to temporary local path. May also use `image_picker` from gallery. |
| **Local temp path** | Image lives in Flutter's temporary directory only — no persistent storage. |
| **Upload request** | `multipart/form-data` POST to `POST /v1/analysis/outfit` (or `/hairstyle`/`/grooming`). The `image` part contains the file bytes. |
| **Media reference** | Backend performs **upload-then-insert** (TRX-1): blob → object storage → `MediaRef` → `analysis_runs.input_media`. The Flutter side never sees the `MediaRef`. |
| **Ownership** | `user_id` from auth token — owner-scoped. The signed URL is scoped to `users/{user_id}/scans/{run_id}/`. |
| **Temporary vs persistent** | Image is **temporary** on Flutter side (local temp path). On the backend, the blob is stored in object storage with **auto-expiry** unless the user saves the run as a saved look. Unsaved scan media auto-expires (retention job). |
| **Allowed media type** | `image/jpeg`, `image/png`, `image/webp` — validated at `/complete` (M16) and pre-upload (S-1/S-2). |
| **Size constraints** | Pre-upload check: `413/422 MEDIA_FAILURE` if `maxBytes` exceeded. Limit aligned with vision model input (e.g. ≤ 20 MB). |
| **Lifecycle** | - Capture → upload → run creation → processing → completion → result retrieval - unless user saves the look, unsaved scan media auto-expires after analysis/30 days. |
| **Deletion expectations** | No user-facing DELETE for scan media. Auto-expire by retention job. Only account erasure (TRX-8) removes scan blobs cascaded from `users` delete. |

### 4.2 Media Reference Structure (`MediaRef`)

The `input_media` column stores a `MediaRef` value object:
...
- **`width`/`height`**: Optional display metadata.
- **`sizeBytes`**: Optional file size.
- **`contentHash`**: SHA-256 hash for integrity verification — never logged or echoed.
- **`isGenerated`**: `false` for captured images; `true` for AI-generated visuals.
- **`uploadedAt`**: Timestamp of upload completion.

### 4.3 Ownership and Privacy

- **Image bytes never logged** — only the `MediaRef` travels in API responses/errors.
- **Image bytes never echoed** in error responses — only `MediaRef` fields (MS10.3, ER-4).
- **HTTPS only** for all image transmission.
- **Owner-only access** — every run is scoped to the caller's `user_id`; another user's `run_id` → `404`.
- **No permanent image retention** unless the user saves the run as a `SavedLook` (which snapshots the `result`, not the raw image).

---

## STEP 5 — API CONTRACT

Design the minimum API required for the REAL Appearance Scan. Prefer existing endpoints and patterns.

### 5.1 Existing Endpoints — Reused

| Endpoint | Method | Purpose | Reuse |
|---|---|---|---|
| `GET /v1/analysis/runs/{run_id}` | auth | Read one run | Reads appearance scan run status/result |
| `GET /v1/analysis/runs` | auth | List runs | Lists user's appearance scan history |
| `POST /v1/analysis/hairstyle` | auth | Create hairstyle run | **Extended** to accept image + produce appearance attributes (faceShape/skinTone/bodyType/styleType) in result |
| `POST /v1/analysis/grooming` | auth | Create grooming run | Accepts options JSON (no image needed) |
| `POST /v1/auth/*` | public/auth | Auth | Required for owner scoping |
...
| **REQUEST** | `multipart/form-data`:
  ...
  - `faceProfileRef?` (optional): UUID — run recommendation-only pass over stored FaceProfile (profile-only mode, no image)
...
| **OWNERSHIP** | `user_id` from auth token; owner-scoped (OW-1); 404-not-403 on run read |
...
### 5.3 API Contract — Image Submission (S-1)
...
Request body:
- image: <file> (required; outfit photo; the evidence)
- faceProfileRef?: <UUID> (optional; profile-only pass)
- [typed context fields] (optional; catalog §12.11)

Response: 202 { "run_id": "7a2b3c..." }
```

The backend:
1. Validates `image` content-type/size → `413/422 MEDIA_FAILURE` if invalid.
2. Performs **upload-then-insert** (TRX-1): blob → object storage → `MediaRef` → `analysis_runs.input_media`.
3. Creates `analysis_run` row: `user_id`, `run_type="outfit"`, `status="pending"`, `engine_version="vision-v1"`, `input_media=<MediaRef>`.
4. Returns `202 {run_id}`.

### 5.4 API Contract — Hairstyle with Image (S-2)
...
- faceProfileRef?: <UUID> (optional; profile-only pass over stored face attributes)

Response: 202 { "run_id": "..." }
```

- **With image**: The run does face analysis → appearance attributes (faceShape/skinTone/bodyType/styleType) → hairstyle recommendations in one async run. Completion applies attributes to `styleProfile` via TRX-6.
- **Without image (profile-only, faceProfileRef present)**: Recommendation pass over already-stored current profile (no new face attributes). Same endpoint, different input.

### 5.5 API Contract — Polling and Result Retrieval (S-5/S-6)
...
- **List runs** for the authenticated user, paged, with optional `?run_type=` filter.
- **Summary rows** omit `result`/`error` (PR-5, no history detail leak).

### 5.6 API Contract — Grooming (S-3)
...
{
  "options": {
    "faceShape": "oval",       // from GroomingOption vocab
...
### 5.7 Alignment with Existing Conventions

- **Naming**: `/v1/analysis/outfit` follows the existing `analysis/` prefix pattern (endpoints 36–40 in API_INVENTORY.md).
- **Response shape**: `202 {run_id}` matches the existing `AsyncAccepted` pattern (API-41/42).
- **Error codes**: Consistent with the 12-category taxonomy (VALIDATION_ERROR=422, MEDIA_FAILURE=413/422, etc.).
- **Auth**: Bearer → user_id via deps.py (OW-1, 404-not-403).
- **Async pattern**: `202 + run_id` → poll `GET /analysis/runs/{run_id}` (C-10, API-40).
- **Idempotency**: Never idempotent for analysis submissions (§11).
...
The `analysis_runs.result` JSONB contains the **Analysis Result** — the immutable snapshot written at completion (TRX-5). It is the structured output of the analysis pipeline.
...
| `recommendations.top` | dict | not nullable | Single best recommendation (HairstyleRecommendation/GroomingRecommendation snapshot) |
| `recommendations.alternatives` | list[dict] | nullable | Alternate recommendations |

### 6.3 Persisted User Profile (separate from result)

The **Persisted User Profile** is `user_state.style_profile` — the current face profile projection. It is updated at completion via TRX-6 (separate transaction from run completion).
...
<function=todowrite>
<parameter=todos>
[{"content": "Read and understand all source documentation files", "priority": "high", "status": "completed"}, {"content": "Step 1-11: Design Appearance Domain Data, Database, Analysis Run, Media, API, Result, Decision Engine Integration, Versioning, Error, Security, Test Contracts", "priority": "high", "status": "completed"}, {"content": "Create APPEARANCE_SCAN_DATA_CONTRACT.md", "priority": "high", "status": "completed"}, {"content": "Create APPEARANCE_SCAN_API_CONTRACT.md", "priority": "high", "status": "completed"}, {"content": "Create APPEARANCE_SCAN_IMPLEMENTATION_PLAN.md", "priority": "high", "status": "in_progress"}, {"content": "Final review and validation", "priority": "medium", "status": "pending"}]