# STEP 7 — First Production Vertical Slice: Hairstyle Recommendation
...
> (`backend/` + `newproject/flutter_application_1/`) against the STEP 2–6
...
Implement **one complete production flow** for hairstyle recommendations:
...
| --- | --- | --- |
| `POST /v1/analysis/hairstyle` (#37) | UC-25/26 | Submit a hairstyle analysis request → `202 {run_id}` (async, never idempotent). |
...
The **Decision Engine** runs rules-first (DE-0). AI enrichment is additive and
text-only via the existing Ollama seam (`app/ai/llm_backend.py`) — structure,
scores, and ranking never come from the LLM.
...
- `CURRENT_STATE.md` (STEP 6 COMPLETE — ready for implementation).
...
- STEP 5: `docs/backend/FASTAPI_ARCHITECTURE_V1.md`, `APPLICATION_USE_CASES.md`
  (UC-6/15/25/26), `DECISION_ENGINE_ARCHITECTURE.md` (stages 5–8),
...
- Hairstyle feature is **fully mock-driven**:
  - `data/hairstyle_mock_data.dart` — `HairstyleRecommendation`
    (id/name/description/matchScore/reasons/stylingTips/maintenance/bestFor)
    and `HairstyleAnalysisResult` (faceShape/skinTone/styleDna/top/
    alternatives), 4 catalog looks (`textured_quiff`, `classic_pompadour`,
    `side_part`, `brushed_up_undercut`) — the wire DTO must mirror this.
  - `presentation/face_scan_screen.dart` — placeholder (no real camera
    capture; `FacePreviewPlaceholder` + mock `FaceScanCheck` rows + "Scan
    Face" button → push processing).
  - `presentation/face_processing_screen.dart` — **fake timers** play the 5
    mock `HairstyleProcessingStage`s, then
    `context.replaceNamed(RouteNames.hairstyleResult)`.
  - `presentation/hairstyle_result_screen.dart` — renders `HairstyleAnalysisResult.mock`;
    "Try Another" → `replaceNamed(hairstyle)`; "Save Style" → snackbar only.
  - `presentation/hairstyle_details_screen.dart` — renders a passed-in
    `HairstyleRecommendation`; "Try This Style" → snackbar only.
  - `presentation/widgets/hairstyle_widgets.dart` — `HairstyleCard`
    (65% image / 35% content via `FansiHeroCard`), `FacePreviewPlaceholder`,
    `HairstyleCheckIndicator`, `HairstyleStageIndicator`.
- Routing (go_router, untouched): `/stylist/hairstyle/processing/result/details`
  (`app_router.dart:233-265`); details receives the recommendation via
  `state.extra`.
- HTTP pattern to reuse: `features/assistant/data/assistant_client.dart`
  (base URL via `--dart-define=ASSISTANT_BASE_URL`, `http` package, timeout,
  returns `null` on failure → offline fallback) +
  `domain/assistant_service.dart` (ChangeNotifier orchestration).
- Tests: 30 widget/unit test files incl. `test/hairstyle_{scan,processing,
  result,details}_screen_test.dart`.

---

## 4. Documented conflicts (rule 16 — before changing code)

These are conflicts between the accepted architecture/contract and what STEP 7
...
pipeline land. STEP 7 explicitly implements this surface as the first
production slice. **STEP 7 therefore IS the milestone that mounts these
...
endpoints #37/#39/#40/#23. The real app and backend have **no auth**. Per
...
`POST /v1/analysis/hairstyle` takes multipart `image` or `faceProfileRef`
(profile-only pass). The M16 media pipeline is sealed until MS10.3, and the
Flutter face-scan screen has no real image capture today. Resolution proposed:
the first slice uses the **profile-only pass (`faceProfileRef`)** — the
recommendation re-rank path (contract §5.7 H-7, "regenerate" form) — grounded
on the stored `user_state.style_profile`; face-image upload is **additive** and
stays behind MS10.3. This preserves honesty (no fake 200, no fabricated face
analysis) and the contract's shapes. See §5.D2.
...
milestone and **out of scope for STEP 7** (rule 1/8: do not rewrite the
...
| --- | --- | --- | --- |
| **D1** | **Auth seam (C-2)** | Dev-identity bearer seam in `deps.py`: token → seeded dev `user_id`; owner scoping (404-not-403) fully enforced. | Unblocks mounting #37/#39/#40/#23 now; D-AUTH-1 lands later as an additive swap behind the same `deps.py`. No contract change. |
...
| **D4** | **AI enrichment scope** | Rules-only engine is the deliverable; optional Ollama **text-wording** enrichment of `description` reuses `llm_backend.py` (never structure/scores). | DE-0 + AI-0 honesty; AI "where available" = the existing seam. |
| **D5** | **Save UI behavior** | "Save Style" / "Try This Style" POST `#23` with an `Idempotency-Key`; on success keep the existing snackbar confirmation. | Minimal UI change (button action only), preserves design (rule: UI safety). |
...
```
Flutter HairStyleFlow (scan → processing → result → details)
   │  POST /v1/analysis/hairstyle  {faceProfileRef}            (multipart not used yet, D2)
   ▼
api/routers/analysis.py  ── deps.py (dev seam, D1) → user_id
   │  UC-25/26 (application/analysis.py)
   ▼
AnalysisRunRepository (infrastructure/db)     INSERT analysis_runs (pending)   [TRX-5 start]
   │
DecisionEngine (domain/services/analysis_rules.py — stage order)
   │  ContextBuilder(catalog: looks + user_state.style_profile)
   │  → CandidateGeneration(catalog hairstyle looks)
   │  → Scoring(face-shape switch, deterministic weights)
   │  → Ranking(top + alternatives)
   │  → Explanation(grounded reasons from catalog)
   │  → Recommendation(HairstyleAnalysisResult DTO)          [rules-first, DE-0]
   │  optional LLM wording enrichment via llm_backend seam   [D4]
   ▼
guarded completion UPDATE analysis_runs SET status='completed', result=…   [TRX-5 write-once]
...
Flutter polls GET /v1/analysis/runs/{run_id} → 200 {run_id,status,result{appearance,recommendations{top,alternatives}}}
   ▼
Flutter result/details screens render fetched data (reuse HairstyleCard → FansiHeroCard)
   ▼
Flutter "Save" → POST /v1/looks/saved {lookId,title,sourceContext:"hairstyle",snapshot} + Idempotency-Key
...
**Endpoints mounted in this slice:** `#37`, `#39`, `#40`, `#23`.
**Endpoints intentionally NOT mounted:** `#35` (feedback, gated), M16 media
(sealed), `#6` `GET /v1/users/me` (only referenced by the engine through the
repository — no new route unless needed).
...
- Add a `postgres` service to `backend/docker-compose.yml` (env: DB name/user/
  pass, port 5432).
- Create `backend/app/infrastructure/db/` — engine/session factory +
  `Base` + dependency. Alembic scaffolding (`alembic.ini`, `env.py`,
  `migrations/versions/`).
- **Validation gate:** `.venv/bin/python -m pytest -q` — existing 19 tests
  still green (no app code touched); Postgres container boots.

### Stage 1 — Backend: knowledge catalog (M5 subset, K9.1)
- Extend `app/data/catalog.py` (or seed `app/infrastructure/external/
  knowledge.py`) with the full 4-look hairstyle catalog mirroring
  `hairstyle_mock_data.dart` (id = `looks.code`, name, description, score seed,
  reasons, stylingTips, maintenance, bestFor) — keep `TEXTURED_QUIFF`/
  `CLASSIC_POMPADOUR` shapes intact so the assistant surface is unchanged.
- Alembic migration: create **knowledge tables** `looks`, `run_types`,
  `signal_types` + seed `looks` rows + `run_types('hairstyle')` +
  `signal_types('look_saved')` (exact STEP-4 shapes).
- **Validation gate:** migration applies to a fresh DB; a seed re-run is
  idempotent; existing 19 tests green.

### Stage 2 — Backend: decision engine (M6 domain, rules-first)
- `app/domain/` (pure Python, no FastAPI/SQLAlchemy/HTTP — BA-3): typed value
  objects mirroring the wire (`HairstyleRecommendation`, `AppearanceProfile`),
  and `app/domain/services/analysis_rules.py` implementing
  **ContextBuilder → CandidateGeneration → Scoring → Ranking → Explanation →
  Recommendation** for hairstyle only (per-task composition, no empty framework).
  Scores from the seeded catalog constants; face-shape switch mirrors
  `tools.recommend_hairstyle`; reasons grounded in catalog text (never invented).
- **Validation gate:** new unit tests for the engine stages (see §11);
  19 existing tests green.

### Stage 3 — Backend: analysis run API (M12, UC-25/26)
- `app/domain/ports/` (repositories, `KnowledgeSource` port) +
  `app/infrastructure/db/repositories.py` (`AnalysisRunRepository` with the
  guarded write-once completion, `UserStateRepository` read).
- `app/application/analysis.py` — `CreateHairstyleRun` + `GetAnalysisRun` +
  `ListAnalysisRuns` use cases (TRX-5 boundary).
- `app/api/deps.py` (dev seam, D1), `app/api/errors.py` (12-category mapper),
  `app/api/schemas/analysis.py` (the canonical `AnalysisRun` DTO incl.
  `input_media?` per ¶7.I-1), `app/api/routers/analysis.py`
  (`POST /v1/analysis/hairstyle` → `202 {run_id}`; `GET /v1/analysis/runs/
  {run_id}` → bare `AnalysisRun`, owner-only 404; `GET /v1/analysis/runs` →
  `{items,page,page_size,total}` summary, no `result`). In-process synchronous
  completion (BJ-1, no Celery).
- Mount routers in `main.py`. `POST /v1/assistant/chat` + `/health` untouched.
- **Validation gate:** new pytest API tests (202/200/404/422 shapes, write-once
  guard, owner scoping); existing 19 green; `dart format`/`flutter analyze` on
  the unchanged app still clean.

### Stage 4 — Backend: save + feedback signal (M7, UC-15)
- `app/infrastructure/db/repositories.py` — `SavedLookRepository` +
  `LearningSignalRepository`.
- `app/application/saved_looks.py` — `SaveRecommendation` use case.
- `app/api/schemas/saved_looks.py` + `app/api/routers/looks.py`
  (`POST /v1/looks/saved` → `201 SavedLook`, **`Idempotency-Key` required**,
  TRX-3: saved_looks INSERT + learning_signals `look_saved` INSERT commit
  together; `source_run_id` SET NULL provenance when the producing run is
  named).
- **Validation gate:** pytest for TRX-3 (both rows or neither), idempotency
  (replay returns original), 404 look/409 duplicate/422; existing 19 green.
...
- `lib/features/hairstyle/data/hairstyle_models.dart` — wire DTOs
  (`AnalysisRun`, `HairstyleRecommendation` fromJson — mirroring the mock
  fields + `matchScore`), per the `flutter-implement-json-serialization` skill.
- `lib/features/hairstyle/data/hairstyle_client.dart` — mirrors
  `assistant_client.dart` (same base URL convention, `http`, timeout, safe
  failure): `submitHairstyleAnalysis(faceProfileRef)`, `getAnalysisRun(id)`,
  `listRuns()`, `saveLook(...)` with `Idempotency-Key`.
- `lib/features/hairstyle/domain/hairstyle_service.dart` — ChangeNotifier
...
### Stage 7 — Flutter: wire the hairstyle flow (UI-safety-compliant)
- `face_processing_screen.dart` — replace the fake timer with the real
...
- `hairstyle_result_screen.dart` — render the fetched
  `HairstyleAnalysisResult` instead of `HairstyleAnalysisResult.mock`; "Save
  Style" → `service.saveLook(...)` (keep snackbar confirmation). Accept the
...
- `hairstyle_details_screen.dart` — "Try This Style" → `service.saveLook(...)`.
- `app_router.dart` — only the additive extra-passing for `hairstyleResult`
  (rule 4: no routing replacement; global navigation untouched).
- **No changes** to scan screen, widgets, theme tokens, card proportions.
- **Validation gate:** `flutter analyze` clean; updated + new hairstyle widget
  tests (see §11); full `flutter test` passes.

### Stage 8 — Save + feedback verification, end-to-end
- Manual E2E: backend up + Postgres up + app → hairstyle flow → result → save →
...
| `backend/app/data/catalog.py` | extend hairstyle catalog to 4 looks (Stage 1) |

**Flutter**
| File | Change |
| --- | --- |
| `lib/features/hairstyle/presentation/face_processing_screen.dart` | real submit/poll instead of fake timers |
| `lib/features/hairstyle/presentation/hairstyle_result_screen.dart` | render fetched result; wire Save Style |
| `lib/features/hairstyle/presentation/hairstyle_details_screen.dart` | wire Try This Style (save) |
| `lib/app/router/app_router.dart` | additive: pass fetched result via `state.extra` to `hairstyleResult` |
...
| `app/infrastructure/db/repositories.py` | infra | `AnalysisRunRepository` (write-once guard), `SavedLookRepository`, `LearningSignalRepository`, `UserStateRepository` read |
| `app/infrastructure/external/knowledge.py` | infra | `KnowledgeSource` adapter seeded from `catalog.py` (K9.1) |
| `app/domain/ports/repositories.py`, `app/domain/ports/external.py` | domain | repository + `KnowledgeSource`/`AIProvider` ports |
| `app/domain/value_objects.py` | domain | `HairstyleRecommendation`, `AppearanceProfile`, snapshot value objects (mirror wire) |
| `app/domain/services/analysis_rules.py` | domain | the hairstyle decision-engine stages (rules-first) |
| `app/application/analysis.py` | application | UC-25/26 + run reads use cases |
| `app/application/saved_looks.py` | application | UC-15 use case |
...
| `app/api/routers/analysis.py`, `app/api/routers/looks.py` | api | `#37/#39/#40`, `#23` |
| `alembic.ini`, `alembic/env.py`, `migrations/versions/0001_*.py` | infra | STEP-4 schema for the slice |
| `backend/tests/test_analysis_api.py`, `test_analysis_rules.py`, `test_saved_looks.py`, `test_db_session.py` | tests | see §11 |

**Flutter**
| File | Purpose |
| --- | --- |
| `lib/features/hairstyle/data/hairstyle_models.dart` | wire DTOs (`AnalysisRun`, recommendation fromJson/toJson) |
| `lib/features/hairstyle/data/hairstyle_client.dart` | HTTP client (mirrors `assistant_client.dart`) |
| `lib/features/hairstyle/domain/hairstyle_service.dart` | submit/poll/save orchestration (ChangeNotifier) |
| `lib/features/hairstyle/data/hairstyle_offline.dart` | deterministic offline fallback (optional, mirrors `offline_assistant.dart`) |
| `test/hairstyle_models_test.dart`, `test/hairstyle_client_test.dart`, `test/hairstyle_service_test.dart` | see §11 |

### 8.3 Reusable code (do not duplicate)
- Backend: `app/data/catalog.py` (knowledge seed; hairstyle look content +
  stable codes), `app/ai/llm_backend.py` (AI seam, `is_available()`/
  `enrich_reply`, degrade-safe), `app/ai/tools.py:recommend_hairstyle`
  (face-shape scoring seed), `app/ai/intent.py` (unchanged), `app/models/
  schemas.py` (untouched; assistant DTOs frozen).
- Flutter: `assistant_client.dart` (client pattern incl. base-URL define),
  `offline_assistant.dart` + `assistant_service.dart` (service pattern),
  `hairstyle_mock_data.dart` (the 4-look catalog the wire mirrors),
  `hairstyle_widgets.dart` (`HairstyleCard`, stage/check indicators),
  `FansiHeroCard`/`FansiImageWell`/`FansiBadge`/`FansiButton` (65/35 card),
  `FansiErrorView`/`FansiLoadingView`, theme tokens (Digital Atelier),
  go_router extras pattern already used by details/grooming.
...
| **Result screen refactor may break existing widget tests** (`hairstyle_result_screen_test.dart` asserts on `HairstyleAnalysisResult.mock`). | Router extra falls back to the mock when no result is provided; update tests to pump with a fetched result while preserving their assertions. |
...
| **UI safety** — risk of redesigning the hairstyle screens. | Only the data source + button actions change; widgets/tokens/cards/navigation untouched; existing tests preserved. |
...
**Backend (`pytest`)**
1. `test_analysis_rules.py` — engine: candidates from catalog; scoring by face
...
2. `test_analysis_api.py` — `POST /v1/analysis/hairstyle` → 202 `{run_id}`;
...
1. `test/hairstyle_models_test.dart` — `AnalysisRun`/recommendation
   fromJson/toJson round-trip against the contract example (§4.3).
2. `test/hairstyle_client_test.dart` — submit returns run_id on 202; poll maps
   200 pending/completed/failed; non-200/network failure → null (offline path).
3. `test/hairstyle_service_test.dart` — submit→poll→result sequence; save sends
   `Idempotency-Key`; failure falls back to offline/mock.
4. Update `test/hairstyle_processing_screen_test.dart` — processing uses real
   poll flow (mock the service); `test/hairstyle_result_screen_test.dart` —
   renders a fetched result and save calls the service; `test/hairstyle_details_
...
- No full 8-stage engine framework — only the hairstyle task's stages (no
...
`newproject/flutter_application_1/lib/features/hairstyle/**`, assistant data
layer, router, tests) + STEP 2–6 docs (domain model, table definitions,
architecture, API contract + hairstyle module contract). **No code changed.**
