...
| Cold-start users: `setFace()` called after face scan stores face profile | OBSERVATION | `FaceProcessingScreen._start()` now calls `LearningService.instance.setFace()` with analysis result face data; enables real backend analysis on subsequent hairstyle runs | Medium (0.6) | Whether face profile storage persists across app restarts | Verify face profile persists in local store |
| `recommendation_selected` emitted from result screen | FACT | `HairstyleResultScreen` emits `recommendation_selected` with `action: save` (Save Style) or `action: dismiss` (Try Another), including `recommendation_id` and `confidence_at_selection` | High (≥0.9) | — | — |
| `recommendation_saved` emitted after save completes | FACT | `HairstyleResultScreen._saveStyle()` emits `recommendation_saved` with `save_success: ok`, `idempotency_key`, `look_saved_signal_committed`, `snackbar_shown` | High (≥0.9) | — | — |
| Confidence score guidance added to result screen | OBSERVATION | Small text "Confidence score based on your face shape analysis" added near match percentage; clarifies score represents confidence based on available face-shape data without implying guaranteed outcomes | Medium (0.6) | — | User feedback on clarity |
| Save button clarification added | OBSERVATION | Small text "Save Style → saved to profile for later reference" added between action buttons; clarifies save purpose without adding incentives, badges, or gamification | Medium (0.6) | — | User feedback on usefulness |
